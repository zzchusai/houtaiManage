<template>
	<div class="main">
		<div class="main-top zoom">
			<div class="lf">首页管理 / <span class="zClr-999">节目/ </span><span class="zClr-999">评论</span></div>
		</div>
			<!--评论页内容-->
				...
				<div v-if="oCommentData.comment">
					<div v-for="(item,index) in oCommentData.comment"  :key="index" class="sf-back-layout">
						...
						<div class="xbox">
							<div class="xbox-1 ml15">
								<div class="sf-font16">
									<span class="fb">{{item.user_nickname}}</span>
							...
							<div v-for="(items,index) in item.children" :key="index">
								<div class="sf-comment-child sf-back-layout mt10">
									...
									<div class="xbox">
										<div class="xbox-1 ml15">
											<div class="sf-font16">
												<span class="fb">{{items.user_nickname}}</span>
								...
								<div v-for="(item,index) in aUser" :key="item.id" class="xbox-1-3 mt20">
									<el-radio v-model="ModelForm.user" :label="item" class="fl sf-radio">
										<div class="sf-user-select fr">
											<img :src="item.avatar"/>
											<span class="sf-user-name">{{item.nickname}}</span>
										</div>
									</el-radio>
								</div>
							</div>
...
//            封装的弹出确定取消事件
			...
			sfConfirm(){
				...
				if(self.turnBtnState){
					...
					self.$http.post(self.HOST+'/comment/save',data).then(function(data){
			...
			sendComment(tag,item,index){
				...
				switch (tag)
				{
					case "one":
						data = {
							type:1,
							program_id:self.iCommentActiveID,
							user_id:self.oActiveUser.id,
							user_nickname:self.oActiveUser.nickname,
...
//            二级回复按钮点击事件
			...
			render(){
				...
				self.$http.post(self.HOST+'/comment',data).then(function(data){
					if(data.data.status == 1){
