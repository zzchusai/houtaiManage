<template>
    <div class="sf-index fl sf-main">
        <div class="main-top zoom pl30">
            <div class="lf">首页管理 / <span class="zClr-999">文章管理</span></div>
        </div>
        <div class="search-btn ml30" >
            <div class="butn btn-box btn-normal" style="margin-top: 5px;line-height: 32px;" @click="gotoAddArticle">上传文章</div>
        </div>
        <div class="sf-table-page">
            <table>
                    ...
                    <tbody>
                    <tr style="border-top:0;cursor: pointer;border-bottom:1px solid #f1f1f1;" v-for="item in tableData" @click="gotoDetailsArticle(item.id)">
                        <td>{{item.id}}</td>
                        ...
                        <td>
                            <span class="operation" style="line-height: 48px;font-size: 14px;" @click.stop="gotoDeitArticle(item.id)">编辑</span>
                            <span v-if="item.is_show === 1" class="sf-color-gray operation delete" style="line-height: 48px;font-size: 14px;"@click.stop="delArticle()">删除</span>
    ...
    export default {
        data() {
            return {
                ...
                sfMessage:"是否删除该条文章？",
                articleId:0,
                titles: ['文章ID','状态','标题','评论数','作者','最后编辑时间','操作'],
                tableData:[],
                page: true,
                cur_page:1,  //分页当前页参数
                totalpages:0,//分页总页数
        ...
        methods: {
            ...
            gotoDeitArticle(id){
                ...
                self.$router.push('/editArticle?id='+id);
            },
            gotoDetailsArticle(id){
                ...
                self.$router.push('/detailsArticle?id='+id);
...
//          分页currentPage 改变时会触发
            ...
            render(num){
                ...
                self.$http.post(self.HOST+'/article',data).then(function(data){
            ...
            sfConfirm(){
                ...
                let data={id:self.articleId};
                self.$http.post(self.HOST+'/article/delete',data).then(function(data){
                    if(data.data.status == 1){
                        Toast.makeText('删除成功');
                        self.render(1);
                        self.bModel = false;
