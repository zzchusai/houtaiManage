    ...
    <div class="main">
        <div class="main-top zoom">
            <div class="lf">首页管理 / <span class="zClr-999">banner管理/</span>
                <span class="zClr-999">编辑banner</span>
            </div>
        </div>
        <div class="page mr30">
            <div class="page-info">
                <el-form  style="min-height: 500px;" label-width="75px"  v-model="addBannerForm" :rules="rules" class="mt10">
                    ...
                    <el-form-item label="选择类型:">
                        <el-select style="width: 300px;"  v-model="addBannerForm.type" placeholder="请选择" @change="selChange">
                    ...
                    <el-form-item label="banner:">
                        <el-upload action='http://upload-z0.qiniu.com'
                            ...
                            <div class="fl mt10">
                             ...
                             <span  @click="addPic" type="primary" class="load" style='display:block'>{{load}}</span>
                            </div>
        ...
        methods: {
            ...
            onSubmit() {
                ...
                if (self.turnBtnState) {
                    ...
                    console.log(self.addBannerForm.radis_pic)
                    bannerData = {
                        id:self.$route.query.id,
                        type:self.addBannerForm.type,
                        multi_id:self.addBannerForm.id,
                        title:self.addBannerForm.content,
                        banner_pic:self.addBannerForm.coveImg,
                    };
                    self.$http.post(self.HOST+'/banner/update',bannerData).then(function(data){
                        if(data.data.status === 1){
                            Toast.makeText("修改成功");
                            self.turnBtnState = true;
            ...
            render(){
               ...
               if(window.navigator.onLine==false) {
                ...
                self.$http.post(self.HOST+'/banner/update',data).then(function(data){
                    if(data.data.status == 1){
                        self.addBannerForm.radis_pic = true;
                        self.load = "更换";
                        self.addBannerForm.type = String(data.data.data.type);
                        self.addBannerForm.coveImg = data.data.data.banner_pic;
                        setTimeout(function(){
                            self.addBannerForm.id = data.data.data.multi_id;
                            self.addBannerForm.content = data.data.data.title;
                        }, 100);
                    }else{
                        alert(data.data.msg);
                    }
