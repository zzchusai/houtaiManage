<template>
	<div class="member main">
		<div class="member-top main-top zoom">
			<div class="member-title lf">会员用户管理 / <span class="zClr-999">普通会员列表</span></div>
	...
	export default {
		data(){
			return {
				...
				type: [{value: "1", label: "用户名"}, {value: "2", label: "手机号"}, {value: "3", label: "ID"}],
		...
		methods:{
			...
			checkUser(id) {
				...
				if(self.YHXQ){
					self.$router.push('/memberInfo?id='+id);
				}
			},
			exportList(){
				let self = this;
				...
				if('PTYHDC'.isLimit()) {
					...
					window.open(self.HOST+'/user/export?&ACCESSTOKEN='+AccessToken+'&keywords='+self.form.name+'&style='+self.form.type);
				} else {
...
<style scoped>
	.data:hover{background: #f1f1f1;cursor: pointer;}
	.model-img{width:calc(33.33% - 10px);display: inline-block;margin-right:10px;}
	.address{max-width: 160px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;-webkit-line-clamp: 2;-webkit-box-orient: vertical;}
</style>