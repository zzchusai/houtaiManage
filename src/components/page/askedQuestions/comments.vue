<template>
    <div class="main">
        ...
        <div class="sf-comment">
            <!--评论标题-->
            <div class="tr mr20 sf-comment-btn" v-if="reply">
                    ...
                    <div class="xbox">
                        <div class="xbox-1 ml15">
                            ...
                            <div class="sf-font14 mt15">
                                <div class="sf-revert fl mr5" @click="revert(index)" v-if="reply">回复</div>
                                <div v-if="item.childrenNums">
                                    <div v-if="iRevertIcon !== index" class="sf-bottom-btn fl" @click="isShow(index)">
                                        •<span class="ml5" v-if="reply">{{item.childrenNums}}条</span>回复
                                    </div>
                                    <div v-else class="sf-top-btn fl" @click="isHidden(index)">
                                        •<span class="ml5">收起回复</span>
                                    </div>
                                </div>
                                <div class="fr sf-comment-btn " v-if="reply" @click="delComment(item.id)">删除</div>
                                <div class='jinxuan set' @click="setjx(index)" v-if="item.choice==1&&reply">设为精选</div>
                                <div class="jinxuan cancel" @click="canceljx(index)" v-if="item.choice==2&&reply">取消精选</div>
                            </div>
                        </div>
                    </div>
                    <!--一级评论发表框-->
                                ...
                                <div class="xbox">
                                    <div class="xbox-1 ml15">
                                        ...
                                        <div class="sf-font14 mt15" v-xlimit="'WDPLQX'">
                                            <div @click="TwoRevert(index)" class="sf-revert fl mr5" v-if="reply">回复</div>
                                            <div class="fr sf-comment-btn " @click="delComment(items.id)" v-if="reply">删除</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-for="(item,index) in aUser" :key="item.id" class="xbox-1-3 mt20">
                                <el-radio v-model="ModelForm.user" :label="item" class="fl sf-radio">
                                    <div class="sf-user-select fr">
                                        ...
                                        <!-- <img src="../../../../static/img/dsbs.png"  style="z-index:100;position:absolute;bottom:2px;right:-7pxpx;width:15px;height: 15px;" v-if="item.type == 2"> -->
    ...
    export default {
        ...
        methods: {
            ...
            onSelectUser() {
                ...
                if (self.turnBtnState) {
                    self.turnBtnState = false;
                    self.$http.post(self.HOST + '/commentInter/total', {
                        type: 4
                    }).then(function(data) {
                        if (data.data.status === 1) {
