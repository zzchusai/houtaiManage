...
<template>
	<div class="fl sf-main ">
		<div class="main-top zoom pl30">
			<div class="lf">问答 / <span class="zClr-999">问答管理</span> <span class="zClr-999">/违规问答</span>
			</div>
			<div class="frs">
		...
		<div class="menu mr30 ml30 zoom">
			<span class="title" style="float:left;display:inline-block;height:50px">分组:</span>
			<div class="scroll">
		...
		methods: {
			...
			lookAnswer(index, event) {
				...
				if ('WDHD'.isLimit()) {
					let item = this.tableData[index];
					let selectMenuItem = this.menus[this.selectIndex];
					this.$router.push('/interlocutionaskComments?id=' + item.id + "&group_id=" + selectMenuItem.id);
				} else {
					sfLayer.makeText("没有权限查看");
				}
			},
			...
			onQueryName() {
				...
				if (self.searchKeyWord != '') {
					this.$router.push('/interlocutionAnswersDetail?&type=' + 3 + '&nickname=' + self.searchName + '&title=' + self.searchKeyWord);
				} else {
					this.$router.push('/interlocutionAnswersDetail?&type=' + 3 + '&nickname=' + self.searchName);
				}
			},
			//关键词查询
			onQueryKeyword() {
				...
				if (self.searchName != '') {
					this.$router.push('/interlocutionAnswersDetail?&type=' + 3 + '&title=' + self.searchKeyWord);
				} else {
					this.$router.push('/interlocutionAnswersDetail?&type=' + 3 + '&nickname=' + self.searchName + '&title=' + self.searchKeyWord);
				}
			}
