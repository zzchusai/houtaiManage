<template>
	<div class="fl sf-main cell" >
		...
		<div style="overflow:hidden;margin-right:30px" class="fc">
			...
			<span class="rankName" v-if="type!=2">{{item.rankName}}</span>
			 <img src='../../../../static/img/dstx.png' class="tx" v-if="type==2" />
            <!-- <img src='../../../../static/img/kdbtx.png' class="tx" v-if="item.code==2" /> -->
			 <span class="fc" v-if="type==3">(水军)</span>
			<div class="anonymous" v-show="anonymous == 2" style="margin-left:20px">(匿名发布)</div>
			...
			<div class="zoom info">
				<div  style="display:inline-block" v-if='bounty_status!=3'>
...
<script >
	...
	export default{
		...
		created(){
			this.askId = this.$route.query.interlocut_id;
		...
		methods:{
	        ...
	        getDetailData(){
				...
				let data = {
					...
					bounty_status:self.bounty_status
				}
				self.$http.post(self.HOST+'/interlocutionBounty/detail',data).then(function(res){
